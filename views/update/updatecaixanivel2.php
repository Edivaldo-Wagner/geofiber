...
if(isset($_POST['updatedata']))//updatedata
    ...
    $horas = date("h");
     ...
     $segundos = date("s");
        ...
        $query = "UPDATE caixa_nivel_2 SET nome='$nome', splitter='$splitter', modificado_em='$ano-$mes-$dia $horas:$minutos:$segundos', tipo_splitter='$tipo_splitter', cor='$cor', status_implementacao='$status_implementacao', observacao='$observacao', escala='$escala' WHERE id='$id'";
        $query_run = mysqli_query($connection, $query);

        if($query_run)
        {
            echo '<script> alert("Data Updated"); </script>';
            header('Location:http://localhost/admin/views/caixa-nivel-2.php');
        }
        else
        {
