...
if(isset($_POST['updatedata']))//updatedata
    ...
    $horas = date("h");
     ...
     $segundos = date("s");
        ...
        $query = "UPDATE concentrador SET nome='$nome', status_implementacao='$status_implementacao', observacao='$observacao', escala='$escala', modificado_em='$ano-$mes-$dia $horas:$minutos:$segundos' WHERE id='$id'";
        $query_run = mysqli_query($connection, $query);

        if($query_run)
        {
            echo '<script> alert("Data Updated"); </script>';
            header('Location:http://localhost/admin/views/concentradores.php');
        }
        else
        {
